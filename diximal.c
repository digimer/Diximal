...
typedef enum {
	// These are being assigned sequential integer values by the compile (xml_content = 0, ... )
...
// This has to come first as it's a forward decleration. This means that I can reference it in my structures before the unions define the
...
} xml_attr_t;			// This name is use as a short-form for "struct xml_attr_s <name>".
...
struct xml_generic_obj_s {
	xml_obj_type_t obj_type;	// This is a variable of the named type (see in enumeration above)
	union xml_obj * right;	// This points to ourself and points to whatever is to the right, or NULL.
};

struct xml_content_obj_s {
	xml_obj_type_t obj_type;
	union xml_obj * right;
	char * xmldata;		// Contents between the opening tag and the first non-content element.
};

struct xml_cdata_obj_s {
	xml_obj_type_t obj_type;
	union xml_obj * right;
	char * xmldata;		// This is the string between the opening and closing CDATA stanza.
};

struct xml_tag_obj_s {
	xml_obj_type_t obj_type;
	union xml_obj * right;
	char * tagname;		// This is the name of the tag.
	xml_attr_t * attributes;// This is a pointer to a linked list.
	union xml_obj * children;// Pointer to chilren.
};

// Define the union.
typedef union xml_obj {
	xml_obj_type_t obj_type;
	// If I didn't want to use 'struct ...', I could have put 'xml_generic_obj_t' after the decleration above.
...
int main(int argc, char *argv[])
{
	...
	// Create the stack. The '100' means that I can go as far as 100 child tags. This *should* be enough.
	...
	if ((xml_root = parse_xml_content(file_contents)) != NULL)
	{
		// I could use errno.
...
xml_obj_t * parse_xml_content(char * file_contents)
{
	...
	char * start = file_contents; // This will be used to keep track of start of data to store.
	char * end   = file_contents; // This will be used to keep track of the end of data to store.
	
	// Current object
	xml_obj_t * current_object;
/*	if ((current_object = calloc(1, sizeof(xml_obj_t))) == NULL)
	{
	...
	while (*file_contents != 0)
	{
		...
		{
			// Yar
			if ((isalpha(*(file_contents+1)) != 0) || (*(file_contents+1) == '_'))
			{
				// Tag name.
				state_type = xml_tag;
			}
